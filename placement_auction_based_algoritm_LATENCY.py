...
def my_algorithm(parameters):
    ...
    for service_id in range(1, 7):
        ...
        if service is None:
            continue
            # Iterando sobre todas as basestations de rede
                ...
                for network_link in NetworkLink.all():
                    ...
                    if edge_server.has_capacity_to_host(service=service):
                        ...
                        if network_switch == network_link.nodes[0]:
                            ...
                            assert isinstance(base_station.wireless_delay, object)
                            latency = network_link.delay + wireless_delay
                            #print(wireless_delay)
                            cpu_free = edge_server.cpu - edge_server.cpu_demand
                            memory_free = (edge_server.memory - edge_server.memory_demand) / 1024
                            score = abs(1/math.log10(latency)) + abs(math.log10(memory_free)) + abs(
                                math.log10(cpu_free))
